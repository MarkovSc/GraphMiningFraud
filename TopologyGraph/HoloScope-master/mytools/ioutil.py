...
def readedge2squarecscm(ifile, weighted=False, delimiter=' '):
    ...
    with myreadfile(ifile, 'rb') as fin:
        for line in fin:
            coords=line.decode().strip().split(delimiter)
...
def loadedge2sm(ifile, mtype=csc_matrix, weighted=False, dtype=int, delimiter=',', idstartzero=True, issquared=False):
    '''
