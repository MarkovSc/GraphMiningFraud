...
class HoloScopeOpt:
    ...
    def aggregationMultiProp(self, mbs, method='sum'):
        ...
        if method == 'sum':
            ...
            bsusps = list(mbs.values())[0]
            for v in list(mbs.values())[1:]:
                bsusps += v
        elif method == 'rank':
            'rank fusion'
            arrbsusps = []
            for val in list(mbs.values()):
                rb = rankdata(-np.array(val), method=rankmethod)
    ...
    def initfastgreedyMD(self, numSing, rbd):
        ...
        tunit = self.tunit
        print('generate tensorfile with tunit:{}, tbins:{}'.format(tunit,tbindic[tunit]))
