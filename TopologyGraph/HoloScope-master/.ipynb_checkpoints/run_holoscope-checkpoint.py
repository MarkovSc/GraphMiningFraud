...
def loadpreprocess(rootnm, path, inject_K):
    ...
    if os.path.isfile(infreqfile) and \
        ...
        M, (trueA, trueB) = injectFraud2PropGraph( freqfile, ratefile, tsfile,
                acnt, bcnt, goal = goal, popbd=popbd, testIdx = 3, suffix=suffix, inject_K=inject_K)
...
if __name__=="__main__":
    ...
    parser.add_argument('-KJ', '--injectblock',
    ...
    if args.run == 0:
        ...
        rootnm = 'yelp'
        path = './testdata/'
        freqfile, tsfile, ratefile, ftA, ftB = loadpreprocess(rootnm, path, inject_K)
